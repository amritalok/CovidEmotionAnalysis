...
"""
=======
Disclaimer: This script is an adaption from https://gist.github.com/giacbrd/b996cfe2f1d24752f23bd119fdd678f2 (Author: Giacomo Berardi <giacbrd.com>)
...
def which_compression(path):
    ...
    #     print("_MAX_SIGN_LEN", _MAX_SIGN_LEN)
...
def get_open(path, mode, file_type=None, encoding='utf-8'):
    ...
    if file_type == 'csv':
        return io.open(path,'r', encoding=encoding)
    else:
...
def download(in_path, out_path, twitter, responses_store=None, sleep_secs=SLEEP_SECS_DEFAULT,
    ...
    count = 0
    read_csv_mode = False
    with (get_open(in_path, 'rb', which_compression(in_path)) if in_path else sys.stdin) as input_stream:
        split_input_file = in_path.split('.')
        if len(split_input_file) > 0 and split_input_file[-1] == 'csv':
            read_csv_mode = True
            input_stream = csv.reader(input_stream)
